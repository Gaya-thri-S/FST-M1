...
select max(purchase_order) as "Maximum", customer_id from orders group by customer_id;

select max(purchase_amount) as "Maximum", customer_id from orders group by customer_id;

select salesman_id, order_date, max(purchase_amount) as "Maximum" from orders where order_date=To_DATE('2012/08/17', 'YYYY/MM/DD') group by salesman_id, order_date  ;
